...
from src.blueprints.utils import login_required, teacher_required, teacher_or_admin_required
...
def create_grade():
    ...
    try:
        ...
        if student_user and xp_reward > 0:
            result = student_user.add_xp(xp_reward)
            db.session.commit()
            level_up = result.get('leveled_up', False)
